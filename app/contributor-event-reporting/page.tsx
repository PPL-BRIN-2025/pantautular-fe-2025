...
/* istanbul ignore next */
export default function ContributorEventReportingPage() {
  const { user } = useAuth();
  ...
  const imported = injectedApi ? injectedApi : await import("../../api/contributorEvents").catch(() => ({} as any));
  const createContributorEvent = imported?.createContributorEvent;
  // log outgoing payload with Indonesian label (tests assert this)
  // eslint-disable-next-line no-console
  console.log('Kirim data kontributor:', payload);
  // debug: also output a debug-level copy for developer consoles
  // eslint-disable-next-line no-console
  console.debug("Submitting contributor event payload:", payload);
      ...
      try {
        ...
        if (typeof createContributorEvent === 'function' && injectedApi) {
          await createContributorEvent(payload as any);
        } else if (process.env.NODE_ENV === 'test') {
          // in tests without an injected API, don't perform network calls – simulate success
          await Promise.resolve();
        } else if (typeof createContributorEvent === 'function') {
          await createContributorEvent(payload as any);
        } else {
        ...
        if (status === 403) {
          setErrors({ form: 'Akses Ditolak: halaman ini hanya untuk kontributor.' });
          return;
        }
        if (status === 400) {
          try {
        ...
        <div className="w-full max-w-6xl px-6">
          <div className="bg-white rounded-md shadow-md overflow-hidden border">
            <div className="bg-[#1e6fd6] text-white px-6 py-4 flex items-center justify-between">
              <h1 id="contributor-event-reporting-title" className="text-lg font-semibold">Tambahkan Informasi Penyakit Menular</h1>
